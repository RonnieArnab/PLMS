...
export const Navbar = () => {
  ...
  const userIdForNotifs = user?.id || user?.user_id || null;
  ...
  const renderNotifs = () => {
    if (!showNotifs || !userIdForNotifs) return null;
    return (
      <div className="absolute right-4 top-14 z-50">
        <NotificationsPanel initiallyOpen={true} userId={userIdForNotifs} />
      </div>
    );
  };

  return (
    <header className="sticky top-0 z-50 backdrop-blur-lg bg-white/80 dark:bg-gray-900/80 border-b border-gray-200 dark:border-gray-700 shadow-sm relative">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
        {/* Brand */}
        <Link to="/dashboard" className="flex items-center gap-2">
          <div className="p-2 rounded-xl bg-green-100">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm relative">
          {menuItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `relative transition font-medium pb-1 ${isActive
                  ? "text-green-600 dark:text-green-400"
                  : "text-gray-700 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400"
                }`
              }
        ...
        <div className="flex items-center gap-3">
          ...
          {user && (
            <div className="mr-2">
              <NotificationsBadge
                userId={userIdForNotifs}
                onOpen={() => setShowNotifs((s) => !s)}
              />
            </div>
          )}
          ...
          >
            {menuItems.map((item) => (
              <NavLink
                ...
                onClick={() => setMenuOpen(false)}
                className={({ isActive }) =>
                  `block w-full px-3 py-2 rounded-lg text-sm font-medium ${isActive
                    ? "bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300"
                    : "text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-800"
                  }`
                }
