  ...
  const handleSubmit = async () => {
    ...
    try {
      const profileRes = await fetch(
        "http://localhost:4000/api/loan-applications/customer-profiles",
        {
          method: "POST",
      ...
      if (!profileData.success) {
        setSubmitError(
          profileData.message || "Failed to create customer profile"
        );
    ...
    const payload = {
      ...
      interest_rate_apr: formData.product?.base_interest_apr
        ? Number(formData.product.base_interest_apr)
        : null,
      processing_fee: formData.product?.processing_fee_pct
        ? Number(formData.product.processing_fee_pct)
        : null,
      risk_grade: null,
    };
        ...
        <Card className="p-6 space-y-6">
          ...
          <div className="flex justify-between pt-6 border-t border-base-200">
            ...
            {currentStep < steps.length ? (
              <Button onClick={goNext} disabled={submitLoading}>
                Next
              </Button>
            ) : (
              <Button
